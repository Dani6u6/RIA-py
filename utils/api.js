...
export async function upscaleImageWithBackend(imageBase64, options = {}) {
  ...
  try {
    ...
    // 2700000ms = 45 minutos (ajustable según necesidad)
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 2700000);
    ...
    if (error.name === "AbortError") {
      throw new Error(
        "⏱️ Timeout: El procesamiento tomó más de 45 minutos. Intenta con una imagen más pequeña o ajusta el timeout."
      );
    }
